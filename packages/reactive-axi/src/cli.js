...
function createFeedbackNextStep(projectDir, sessionEnded, endedBy) {
  ...
  return `Each prompt's target (when present) carries "clicked" (the literal element that was clicked, resolved to its own real fileName/lineNumber - present even when that location is inside a third-party dependency), "anchor" (the nearest enclosing named component in the application's own code, one level out from "clicked" - present whenever one could be found, whether or not "clicked" itself was vendor code), and "ancestry" (the component names from "clicked" out to "anchor", for broader context on what's nesting what). Use "clicked" for a change/bug that's genuinely about that exact element; use "anchor" (plus "ancestry" for context) when the request is really about the surrounding component, or when "clicked" turns out to be vendor code. If "clicked.unresolved": true, reactive-axi could not find an exact source location at all for that element - typically a Next.js App Router Server Component, whose click target resolves into React's own internal runtime rather than application code; fall back to the target's selector/route and the prompt text to locate the right spot. If "clicked.lineNumber" is null (Vue only), the fileName and componentName are real and resolved, but there's no exact line - Vue's template compiler doesn't emit per-element line metadata by default, so use the fileName and componentName to find the right spot in that file. If "clicked.vendor": true, the clicked element itself is rendered by a third-party dependency (clicked.fileName points inside it, clicked.vendorPackage names the package when known, and clicked.note spells out exactly what to do) - for a change/bug request, never edit that file, it is not part of this project and changes to it will not persist; instead make the change in the application's own code at "anchor" (through props, wrapper markup, or CSS overrides, whichever the requested change actually calls for), using the vendor component's own documented behavior only to understand what it does, not as a place to patch. If "anchor" is null in this situation, no application-level usage could be located automatically - search the app's own source for where it imports or uses the vendor package, or ask the user for more context before assuming where to edit. For a question/comment kind (not a change/bug), just answer using what "clicked"/its vendor note tell you about the component's behavior - do not edit anything. If a request turns out to be about the vendor library's own behavior rather than how this app uses it, say so back to the user rather than guess-patching inside node_modules. The change will hot-reload live in the reviewer's browser automatically - you do not need to do anything else to show it. Do not respond to the user just yet. Now you must run \`reactive-axi poll ${projectDir} --agent-reply "<message for the user>"\`. ${POLL_WAKE_PATH_RULES.join(" ")}`;
}
