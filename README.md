...
| Create React App       | `react-scripts`            | Whatever the installed `react-scripts` scaffolds (CRA itself is in maintenance mode upstream) | File + component, line approximate (CRA's own dev sourcemap - `cheap-module-source-map` - only maps line-level granularity, not columns; see below) |
